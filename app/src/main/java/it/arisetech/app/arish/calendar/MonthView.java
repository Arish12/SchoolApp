...
public abstract class MonthView extends View {
        ...
        public MonthView(Context context, AttributeSet attr, DatePickerController controller) {
            ...
            if(darkTheme) {
                mDayTextColor = ContextCompat.getColor(context, R.color.style_color_primary_dark);
                ...
                mHighlightedDayTextColor = ContextCompat.getColor(context, R.color.btn_default_shadow);
            }
            else {
            ...
            mSelectedDayTextColor = ContextCompat.getColor(context, R.color.blue_500);
            mTodayNumberColor = mController.getAccentColor();
            mMonthTitleColor = ContextCompat.getColor(context, R.color.blue_500);
        ...
        /*sets the weekday for starting of every month */
            //mDayOfWeekStart = mCalendar.get(Calendar.DAY_OF_WEEK);
            mDayOfWeekStart = dc.getFirstWeekDayMonth(mCalendar.get(Calendar.YEAR), mCalendar.get(Calendar.MONTH) + 1);
        ...
        /*
        this returns the total number of days in a month and set it to the number of cells for display
        since nepali date has no fixed pattern that i know if yet
        TODO find a pattern
        so the total number of days is provided by the hashMap containing the list of total days in a month
        between the range of calender
        */
            // mNumCells = mCalendar.getActualMaximum(Calendar.DAY_OF_MONTH);
            mNumCells=dc.noOfDaysInMonth(mCalendar.get(Calendar.YEAR), mCalendar.get(Calendar.MONTH) + 1);
